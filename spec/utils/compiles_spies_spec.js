import CompileSpies from '../../src/utils/compile_spies';
import {List, Map} from 'immutable';

describe('CompileSpies', function(){
  describe('Main Function', function(){
    let creator, CreateItem, GetTitleAndKeys, list, Merger, titleKeys;
    beforeEach(()=>{
      titleKeys = jasmine.createSpy().and.callFake((title)=>{
        if (title === null) return null;
        return {title};
      });

      creator = jasmine.createSpy().and.callFake((title)=>{
        if (title === null) return null;
        return Map({title});
      });
      CreateItem = jasmine.createSpy().and.returnValue(creator);
      Merger = jasmine.createSpy().and.callFake((list, item)=>list.push(item));
    ...
    it('should call creator', function(){
      ...
      ['foo', null, 'bar'].forEach((val, i)=>{
        let calls = creator.calls.argsFor(i);
    ...
    it('should call Merger', function(){
      ...
      [Map({title: 'foo'}), Map({title: 'bar'})].forEach((val, i)=>{
        let calls = Merger.calls.argsFor(i)[1];
